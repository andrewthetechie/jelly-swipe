...
> **RESOLVED by issue #299 (provider split).** The god-object `JellyfinLibraryProvider` was split by role into an async package (`jellyswipe/jellyfin/{client,vault,library,watchlist}.py`). The semantic remnant is cleaned up: `JellyfinVault` owns the single delegate token + delegate user resolution, and `JellyfinWatchlistWriter.add_to_favorites(media_id)` uses the delegate token/user directly — the per-user-token-shaped `user_auth_header`, `resolve_user_id_from_token`, and `add_to_user_favorites` are gone.
...
**Rejected because:** it adds a new env var that 99% of operators (single-user Jellyfin or admin-owned API key) will never set, and the failure mode of _not_ setting it (silently picking `users[0]`) is identical to dropping the feature outright. The cost (new var to document, support, validate) outweighs the benefit (one extra config knob for multi-user-server operators who could equally provision a dedicated user and create the API key under that account).

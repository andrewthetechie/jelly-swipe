...
const CardItemView = React.forwardRef<CardItemHandle, CardItemViewProps>(
    function CardItemView({ cardItem, stackIndex, zIndex, onSwipe }: CardItemViewProps, ref: React.ForwardedRef<CardItemHandle>): JSX.Element {
        ...
        // The single commit path shared by a committed pointer drag (handlePointerUp)
        // and the imperative handle (CardItemHandle.swipe). Both light the matching
        // stamp, fly the card off-screen via exitDistanceFor, and fire onSwipe once,
        // so a button/keyboard press cannot drift apart from a pointer drag.
        const commitSwipe = (
            direction: "left" | "right",
            velocity: number,
            rotation: number,
            ...
            void onSwipe?.(cardItem, direction)
        }
        ...
        // Imperative handle for non-drag swiping (Like/Nope buttons, arrow keys —
        // issue #344). Mirrors the gesture path's top-card-only rule: the drag
        // handlers are only attached when stackIndex === 0, so these no-op for
        // back cards too.
        React.useImperativeHandle(ref, () => ({
            swipe: (direction: "left" | "right") => {
                if (!isTopCard) return
                // A button/key press has no drag velocity, so velocity is 0; no drag
                // distance either, so the exit rotation is a flat 0.

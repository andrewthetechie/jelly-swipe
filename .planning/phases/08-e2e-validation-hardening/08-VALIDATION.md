...
- **Before `/gsd-verify-work`:** `08-E2E.md` exists; `01`–`05` `*-VALIDATION.md` present and `status: complete`; `v1.0-MILESTONE-AUDIT.md` references fresh evidence.
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 08-01-01 | 01 | 1 | E2E narrative | T-08-01 | No secrets in `08-E2E.md` | grep + manual | `test -f .planning/phases/08-e2e-validation-hardening/08-E2E.md` | ✅ | ✅ green |
| 08-02-01 | 02 | 2 | Validation closure | T-08-01 | No secrets in new validation files | grep | `test -f .planning/phases/03-jellyfin-authentication-http-client/03-VALIDATION.md` | ✅ | ✅ green |
| 08-02-02 | 02 | 2 | Validation closure | T-08-01 | No secrets | grep | `test -f .planning/phases/04-jellyfin-library-media/04-VALIDATION.md` | ✅ | ✅ green |
| 08-03-01 | 03 | 3 | Re-audit | T-08-02 | No stale “missing VERIFICATION” claims | grep | `rg "08-E2E" .planning/v1.0-MILESTONE-AUDIT.md` | ✅ | ✅ green |
...
- [x] **N/A — explicit:** Phase 8 does not install pytest or new CI browsers. Wave 0 marked complete with rationale: evidence lives in markdown + compile/grep/docker per `08-CONTEXT.md` D-02.
...
- [x] `08-E2E.md` published with cross-links to `07-VERIFICATION.md` and `03` / `04` / `05` verification files
- [x] `03-VALIDATION.md` and `04-VALIDATION.md` created; `01` / `02` / `05` validation files finalized (`complete` or documented N/A per checklist area)
- [x] Secret-negative checks pass on all new/changed operator-facing markdown
- [x] `v1.0-MILESTONE-AUDIT.md` updated for re-audit
- [x] `nyquist_compliant: true` set in this file’s frontmatter when above are true

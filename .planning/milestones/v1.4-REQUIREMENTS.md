...
- [x] **TEMP-01**: Unraid template uses JELLYFIN_URL instead of PLEX_URL
- [x] **TEMP-02**: Unraid template uses JELLYFIN_API_KEY or JELLYFIN_USERNAME/JELLYFIN_PASSWORD instead of PLEX_TOKEN
- [x] **TEMP-03**: Unraid template includes TMDB_API_KEY masked variable
- [x] **TEMP-04**: Unraid template includes FLASK_SECRET masked variable

### Template UX

- [x] **UX-01**: Unraid template masked fields have no fake placeholder values (blank by default)

### CI Validation

- [x] **CI-01**: CI workflow lints Unraid template to verify variables are a strict subset of recognized app env vars
...
|-------------|-------|--------|
| TEMP-01 | Phase 18 | Complete |
| TEMP-02 | Phase 18 | Complete |
| TEMP-03 | Phase 18 | Complete |
| TEMP-04 | Phase 18 | Complete |
